...
webbrowser.open('https://discord.gg/shots')

while True:
    rpc.update('In the main menu')
    cmd.cls()
    cmd.title(f'Eval {__VERSION__} | Tokens {get.token_count()} | Proxies {get.proxy_count()} | dsc.gg/shots | Made by K1')
    UI().banner()
    UI().menu()
    choicething = Colorate.Horizontal(Colors.red_to_white, f'''
┏━━{os.getlogin()}@Eval
┗━━━> ''')
    ...
    if choice in options:
        ...
        log.log('UI', 'For now report all issues to my dms (poppin10s)', True)
        ...
        if choice in ['01', '1']:
            print(Colorate.Horizontal(Colors.red_to_white, __CHANGELOG__))
            input(Colorate.Horizontal(Colors.red_to_white, 'Enter to continue'))

        elif choice in ['02', '2']:
            print(Colorate.Horizontal(Colors.red_to_white, __FULLCHANGELOG__))
            input(Colorate.Horizontal(Colors.red_to_white, 'Enter to continue'))
