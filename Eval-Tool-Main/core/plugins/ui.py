...
'''
        ...
        banner = pystyle.Colorate.Horizontal(pystyle.Colors.red_to_white, banner)
        print(banner)    


    def menu(self):
        menu = f'''
{'Eval Tool Made By K1 & Ego  |  Discord.gg/fend  |  Discord.gg/shots'.center(self.size)}
{''.center(self.size)}
...
'''
        ...
        menu = pystyle.Colorate.Horizontal(pystyle.Colors.red_to_white, menu)
    ...
    def make_menu(self, options: list, retunrN: bool = False, tabs: int = 0):
        ...
        for index, option in enumerate(options, 1):
            if option != 'Return':
                ...
                option_str = f'<< -> {option}\n'
            thong = Colorate.Horizontal(Colors.red_to_white, f'{tabs}{option_str}')
            print(thong)  
        print('\n')

    def ask(self, question: str, yn: bool = False):  
        if not yn:
            answerthong = Colorate.Horizontal(Colors.red_to_white, f'[{question}] -> ') 
            answer = input(answerthong)
            return answer
        else:
            answerthong = Colorate.Horizontal(Colors.red_to_white, f'[{question}] (y/n) -> ') 
            answer = input(answerthong)
            if answer in ['y', 'Y']:
                return True
            else:
