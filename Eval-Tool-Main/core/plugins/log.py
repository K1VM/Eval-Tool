...
class log:
    ...
    def log(self, main: str = 'UNKNOWN', end: str = 'UNKNOWN', inp: bool = False):
        start = Colorate.Horizontal(Colors.red_to_white, f'[{main}]') 
    ...
    def wip(self):
        main = Colorate.Horizontal(Colors.red_to_white, f'[UI]') 
        end = Colorate.Horizontal(Colors.red_to_white, f'[Work in progress...]') 
        input(f'{self.gettime()} {main}{self.res} -> {end}')

    def good(self, main: str, end: str):
        main = Colorate.Horizontal(Colors.red_to_white, f'[{main}]') 
        end = Colorate.Horizontal(Colors.red_to_white, f'[{end}]') 
    ...
    def warn(self, end: str):
        main = Colorate.Horizontal(Colors.red_to_white, f'[Warn]') 
        end = Colorate.Horizontal(Colors.red_to_white, f'[{end}]') 
        print(f'{self.gettime()} {main}{self.res} -> {end}')

    def locked(self, end: str):
        main = Colorate.Horizontal(Colors.red_to_white, f'[Locked]') 
        end = Colorate.Horizontal(Colors.red_to_white, f'[{end}]') 
        print(f'{self.gettime()} {main}{self.res} -> {end}')

    def liimt(self, end: str):
        main = Colorate.Horizontal(Colors.red_to_white, f'[Limited]') 
        end = Colorate.Horizontal(Colors.red_to_white, f'[{end}]') 
        print(f'{self.gettime()} {main}{self.res} -> {end}')
        
    def hcap(self, end: str):
        main = Colorate.Horizontal(Colors.red_to_white, f'[HCaptcha]') 
        end = Colorate.Horizontal(Colors.red_to_white, f'[{end}]') 
        print(f'{self.gettime()} {main}{self.res} -> {end}')
